...
<template>
  <div class="bg-slate-800 rounded-lg p-6 border border-slate-700 max-w-md mx-auto">
    <div class="space-y-4">
      <div>
        <Label for="flightType" class="block text-sm font-medium text-white mb-2"
          >Type of Flight</Label
        >
        <Select id="flightType" v-model="tripType">
          <SelectTrigger class="w-full bg-white text-gray-900">
      ...
      <div>
        <Label for="startDate" class="block text-sm font-medium text-white mb-2">Start Date</Label>
        <DatePicker id="startDate" v-model="startDate" :min-value="today(getLocalTimeZone())" />
      </div>

      <div>
        <Label for="endDate" class="block text-sm font-medium text-white mb-2">Return Date</Label>
        <DatePicker
          id="endDate"
          v-model="endDate"
